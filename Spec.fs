...
module Spec =
    ...
    let ``Updating product price affects draft order``() =
        ...
        <| fun qty ->
            ...
            total.total * 2m = totalu.total && total.total > 0m && status = Draft
    ...
    [<Property>]
    let ``Recurring invoicing without proration``() =
        Gen.choose (5, 20)
        |> Arb.fromGen
        |> Prop.forAll
        <| fun qty ->
            ...
            total.subtotal = inv.subtotal && total.taxTotal = inv.taxTotal && total.total = inv.total

    [<Property>]
    let ``Next-month prorated billing``() =
        Gen.choose (1, 10)
        |> Arb.fromGen
        |> Prop.forAll
        <| fun s ->

            let pid = addProduct "pro1" 1m false
            ...
            setBillingDay oid s
            let total1 = oid |> getTotal
            setBillingDay oid (s + 1)
            let total2 = oid |> getTotal
            setBillingDay oid (s + 2)
            let total3 = oid |> getTotal
            let r1 = total2.total - total1.total
            let r2 = total3.total - total2.total
            abs (r2 - r1) < 0.003m
